...
Процедура ВалидироватьНаСервере() // BSLLS:Typo-off
	...
	Отбор = Новый Структура("Предупреждение", Истина);
	МассивПредупреждений = РезультатВалидации.Ошибки.НайтиСтроки(Отбор);
	Для Каждого Предупреждение Из МассивПредупреждений Цикл
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = Предупреждение.Представление;
	...
	МассивОшибок = РезультатВалидации.Ошибки.НайтиСтроки(Отбор);
