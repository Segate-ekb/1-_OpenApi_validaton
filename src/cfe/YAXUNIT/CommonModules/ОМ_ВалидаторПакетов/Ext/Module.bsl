    ...
    ЮТТесты
		...
		.ДобавитьТестовыйНабор("СложныеСхемы")
			...
			.ДобавитьТест("Ошибка_ВалидацияПоДискриминатору")
				...
				.СПараметрами(Новый Структура("type, valueA, commonField", "A", 123, "hello world"),
					"OneOfSchemaMismatch",
					"Объект должен соответствовать хотя бы одной схеме")
				.Представление("Дискриминатор_не_указан")
				.СПараметрами(Новый Структура("typo, valueA", "A", "hello world"),
					"MissingRequiredDiscriminatorProperty",
					"<discriminator> ключевое свойство <type> Должно существовать и быть заполнено;")
				.Представление("Дискриминатор_нет_схем_поДискриминатору")
                .СПараметрами(Новый Структура("typo, valueA", "Нет такого", "hello world"),
					"OneOfSchemaMismatch",
					"Объект должен соответствовать хотя бы одной схеме")
КонецПроцедуры
...
Процедура Ошибка_НеЗаполненоОбязательноеСвойство() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"NullPropertyValue",
				"Свойство .* объявлено, и должно быть заполнено, но это не так"))
...
Процедура Ошбика_ОжиданиеОбъекта() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
    			"InvalidType",
				"Передан тип <.*>. Список допустимых типов: <.*Структура.*>"))
...
Процедура Ошбика_ОжиданиеМассива() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
    			"InvalidType",
				"Передан тип <.*>. Список допустимых типов: <.*Массив.*>"))
...
Процедура Ошибка_НекорректныйТип() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
    			"InvalidType",
				"Передан тип <.*>. Список допустимых типов: <.*>"))
...
Процедура Предупреждение_depricated() Экспорт
	...
	ЮТест.ОжидаетЧто(Ошибки)
			...
			.Содержит(ПредикатПоискаОшибок(
				"DeprecatedProperty",
				"Свойство признано устаревшим! Необходимо отказаться от использования в кратчайшее время.")
				.Свойство("Предупреждение").Равно(Истина))
КонецПроцедуры
...
Процедура Ошибка_СтрокаДлиннееМаксимальной() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"MaxLengthRequired",
				"Длина строки превышает максимальную. Максимальная длина <10>"))
...
Процедура Ошибка_СтрокаНеИзПеречисления() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"InvalidEnumValue",
    			"Значение <.*> не найдено среди доступных вариантов. Список доступных значений:"))
...
Процедура Ошибка_РазличныйКвалификаторДаты() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"InvalidDateFormat",
				"Неверный формат даты. Ожидается строка в формате <.*>"))
...
Процедура Ошибка_НекорректныйФормат_РегулярноеВыражение(ИдентификаторФормата) Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"InvalidFormat",
				"Ожидался формат "+ИдентификаторФормата))
...
Процедура Ошибка_ЧислоМеньшеМинимума_неВключая() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"MinValueRequired",
				"Значение <1> должно быть больше <1>"))
КонецПроцедуры
...
Процедура Ошибка_ЧислоБольшеМаксимума_неВключая() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"MaxValueRequired",
				"Значение <10> должно быть меньше <10>"))
КонецПроцедуры
...
Процедура Ошибка_ЧислоМеньшеМинимума() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"MinValueRequired",
				"Значение <0> должно быть больше или равно <1>"))
КонецПроцедуры
...
Процедура Ошибка_ЧислоБольшеМаксимума() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"MaxValueRequired",
				"Значение <100> должно быть меньше или равно <10>"))
КонецПроцедуры
...
Процедура Ошибка_ЧислоКратноЗначению() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"NonDivisible",
    			"Значение <15> должно быть кратно <10>"))
КонецПроцедуры
...
Процедура Ошибка_ЧислоНеЦелое() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"NoneInteger",
				"Некорректный формат значения, число должно быть целым."))
...
Процедура Ошибка_НекорректныйФорматЦелого(ИдентификаторФормата, Значение) Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ПредикатПоискаОшибок(
				"Invalid" + ТРег(ИдентификаторФормата),
				"Некорректный формат значения, число должно быть \d+-битным целым."))
...
Процедура Ошибка_ЧислоЭлементовМеньшеМинимума() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ПредикатПоискаОшибок(
				"MinimumArrayElementsRequired",
				"Количество элементов массива <1> меньше минимального порога <2>"))
...
Процедура Ошибка_ЧислоЭлементовБольшеМаксимума() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ПредикатПоискаОшибок(
				"MaximumArrayElementsRequired",
				"Количество элементов массива <4> больше максимального порога <3>"))
...
Процедура Ошибка_НеУникальныеЭлементы() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ПредикатПоискаОшибок(
				"NonUniqueArrayElements",
				"Значения в массиве не уникальны;"))
...
Процедура Ошибка_ЧислоСвойствМеньшеМинимума() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ПредикатПоискаОшибок(
				"MinimumPropertiesRequired",
				"Количество свойств объекта <1> меньше минимального порога <2>"))
...
Процедура Ошибка_ЧислоСвойствБольшеМаксимума() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ПредикатПоискаОшибок(
				"MaximumPropertiesRequired",
				"Количество свойств объекта <4> больше максимального порога <3>"))
...
Процедура Ошибка_ПроверкаРаботыЗапретаДопСвойств() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				...
				"Не удалось найти свойство <additional> в схеме данных."))
...
Процедура ВалидацияДопСвойств(ИмяПроверяемойСхемы, ЖдемОшибку = Ложь) Экспорт
	...
	Если ЖдемОшибку Тогда
		ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"InvalidType",
				"Передан тип <Число>. Список допустимых типов: <Строка>"));
	Иначе
		ЮТест.ОжидаетЧто(МассивОшибок)
			 .ИмеетДлину(0)
	КонецЕсли;
...
Процедура Ошибка_ВалидацияСтроки() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"InvalidType",
				"Передан тип <.*>. Список допустимых типов: <Строка>"))
КонецПроцедуры
...
Процедура Ошибка_ВалидацияЧисла() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"InvalidType",
				"Передан тип <.*>. Список допустимых типов: <Число>"))
...
Процедура Ошибка_ВалидацияБулева() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"InvalidType",
				"Передан тип <.*>. Список допустимых типов: <Булево>"))
КонецПроцедуры
...
Процедура Ошибка_oneOf() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			.ИмеетДлинуБольше(0)
			.Содержит(ПредикатПоискаОшибок(
				"MultipleOneOfSchemaMatch",
				"<oneOf> Объект должен соответствовать только одной схеме. Индексы валидных схем <0;1>"))
...
Процедура Ошибка_AllOf() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"MissingRequiredProperty",
    			"Отсутствует обязательное свойство <additionalField>"))
...
Процедура Ошибка_ВалидацияСложногоОбъекта() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"MissingRequiredProperty",
				"Отсутствует обязательное свойство <status>"))
...
Процедура Ошибка_ВалидацияNot() Экспорт
	...
	ЮТест.ОжидаетЧто(МассивОшибок)
			...
			.Содержит(ПредикатПоискаОшибок(
				"NotSchemaMismatch",
				"<not> Объект не должен соответствовать схеме, а он соответствует;"))
КонецПроцедуры
...
Процедура Ошибка_ВалидацияПоДискриминатору(ОбъектПроверки, КодОшибки, ШаблонОшибки) Экспорт
...
Функция ПредикатПоискаОшибок(КодОшибки, ШаблонПредставления = Неопределено)
	
	Предикат = ЮТест.Предикат()
		.Свойство("КодОшибки").Равно(КодОшибки);
		
	Если ШаблонПредставления <> Неопределено Тогда
		Предикат
			.Свойство("Представление").СодержитСтрокуПоШаблону(ШаблонПредставления);
	КонецЕсли;
